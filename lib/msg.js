...
const sms = (conn, m) => {
    if (m.key) {
        ...
        m.isGroup = m.chat?.endsWith('@g.us') || m.chat?.endsWith('@lid') || false
        ...
        if (m.fromMe) {
            ...
            if (m.isGroup) {
                m.sender = m.key.participant || m.key.participantPn || m.key.remoteJid
            } else {
                m.sender = m.key.remoteJid || m.key.participant
            }
        }
    ...
    // Newsletter style for all replies
    const newsletterContext = {
        forwardingScore: 999,
        isForwarded: true,
        forwardedNewsletterMessageInfo: {
            newsletterJid: '1203634129500689008@newsletter',
    ...
    m.reply = (teks, id = m.chat, option = { mentions: [m.sender] }) => 
        conn.sendMessage(id, { 
            ...
            contextInfo: { 
                ...newsletterContext,
                mentionedJid: option.mentions || [m.sender] 
            } 
        }, { quoted: m })
    ...
    m.replyDoc = (doc, id = m.chat, option = { mentions: [m.sender], filename: 'undefined.pdf', mimetype: 'application/pdf' }) => 
        conn.sendMessage(id, { 
            document: doc, 
            mimetype: option.mimetype || 'application/pdf', 
            fileName: option.filename || 'undefined.pdf', 
            contextInfo: { 
                ...newsletterContext,
                mentionedJid: option.mentions || [m.sender] 
            } 
